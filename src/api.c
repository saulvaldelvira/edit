...
#define map_confirm(key, n, f, ...) register_mapping((key_ty) { .k = key}, n, make_cmd(f, __cmd_args(__VA_ARGS__) ) )

#define map_confirm_modif(key, n, m, f, ...) register_mapping((key_ty) { .k = key, .modif = m}, n, make_cmd(f, __cmd_args(__VA_ARGS__) ) )
...
#define __args(n, ...) __unwrap_args(n, __VA_ARGS__)
...
__api_func_call(api_line_insert_newline, line_insert_newline)
...
#define direction_cmd(kc, dir) {\
        int cmd = \
        make_cmd(api_move_cursor, \
                __cmd_args( \
                        arg_int(CURSOR_DIRECTION_ ## dir),\
                        arg_int(1) \
                ));\
        register_mapping((key_ty){ .k = ARROW_ ## dir }, 0, cmd);\
        register_mapping((key_ty) { .k = CTRL_KEY(kc) }, 0, cmd);\
}
