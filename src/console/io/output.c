#include "output.h"
#include "buffer.h"
#include "lib/str/wstr.h"
#include <buffer/line.h>
#include "util.h"
#include "state.h"
#include <console/cursor.h>
#include <stdarg.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <wchar.h>

static string_t *buf = NULL;

static void editor_draw_rows(string_t *buf);
static void editor_draw_status_bar(string_t *buf);
static void editor_draw_message_bar(string_t *buf);

static void cleanup(void){
        str_free(buf);
}

static int check_window_size(void){
	int rows, cols;
	get_window_size(&rows, &cols);
	rows -= 2;
	if (rows != state.screen_rows || cols != state.screen_cols){
		for (int i = state.screen_rows; i < rows; i++){
			string_t *str = str_empty();
			vector_append(state.render, &str);
		}
		state.screen_rows = rows;
		state.screen_cols = cols;
		cursor_adjust();
		return 1;
	}
	return 0;
}

static int num_width(int n){
        int max = 1;
        while (n >= 10){
                n /= 10;
                max++;
        }
        return max;
}

static void move_cursor(int x, int y, bool respect_linenr){
        if (buffers.curr->conf.line_number && respect_linenr)
                x += num_width(buffers.curr->num_lines) + 1;
        char move_cursor_buf[32];
        snprintf(move_cursor_buf,
                 ARRAY_SIZE(move_cursor_buf),
                 "\x1b[%d;%dH",
                 y, x);
        str_concat_cstr(buf, move_cursor_buf, ARRAY_SIZE(move_cursor_buf));
}

void editor_refresh_screen(bool only_status_bar){
        ONLY_ONCE(
                buf = str_empty();
                atexit(cleanup);
        )

	if (check_window_size() != 0)
		only_status_bar = false;

	str_concat_cstr(buf, "\x1b[?25l", 6);

	if (only_status_bar){
                // Move to the last two lines (status and message)
                move_cursor(0, state.screen_rows + 1, false);
                updated_status_line();
	}else{
                editor_update_render();
                str_concat_cstr(buf, "\x1b[H", 3);
                editor_draw_rows(buf);
	}

	editor_draw_status_bar(buf);
	editor_draw_message_bar(buf);
	// Restore cursor position
        move_cursor(buffers.curr->rx - buffers.curr->col_offset + 1,
                    buffers.curr->cy - buffers.curr->row_offset + 1, true);

	str_concat_cstr(buf, "\x1b[?25h", 6);
	printf("%s", str_get_buffer(buf));
        editor_log(LOG_INFO, "%s", str_get_buffer(buf));
	fflush(stdout);
	str_clear(buf);
}

static void print_welcome_msg(string_t *buf){
	static char* welcome_messages[] = {
		"Edit -- Terminal-based text editor",
		"Press Alt + H to display the help buffer",
		NULL
	};
	int i;
	for (i = 0; i < state.screen_rows / 4; i++)
		str_concat_cstr(buf, "~\x1b[K\r\n", 6);

	for (char **line = welcome_messages; *line != NULL;){
		char *msg = *line;
		int welcome_len = strlen(msg);
		if (welcome_len > state.screen_cols - 2)
			welcome_len = state.screen_cols - BOTTOM_MENU_HEIGHT;
		int padding = (state.screen_cols - welcome_len) / 2;
                padding -= BOTTOM_MENU_HEIGHT;
		str_concat_cstr(buf, "~ ", 2);
		while (padding-- > 0)
			str_push_char(buf, ' ');
		str_concat_cstr(buf, msg, welcome_len);
		line++;
		str_concat_cstr(buf, "\x1b[K\r\n", 5);
		if (++i >= state.screen_rows)
			break;
	}

	for (; i <= state.screen_rows; i++)
		str_concat_cstr(buf, "~\x1b[K\r\n", 7);
}

void editor_draw_rows(string_t *buf){
	if (buffers.curr->num_lines == 0
	 && buffers.curr->filename == NULL
	 && buffers.curr->dirty == 0
	 && buffers.amount == 1
	 ){
		print_welcome_msg(buf);
		return;
	 }

	for (int y = 0; y < state.screen_rows; y++){
		int file_line = y + buffers.curr->row_offset;
		if (file_line >= buffers.curr->num_lines){
			str_push_char(buf, '~');
		}else{
                        size_t screen_cols = state.screen_cols;
                        if (buffers.curr->conf.line_number) {
                                screen_cols -= num_width(buffers.curr->num_lines) + 1;
                                char lnr_buf[128];
                                int padding = num_width(buffers.curr->num_lines) - num_width(file_line + 1) + 1;
                                snprintf(lnr_buf, sizeof(lnr_buf) / sizeof(lnr_buf[0]),
                                         "%d%*s", file_line + 1, padding, "");
                                str_concat_cstr(buf, lnr_buf, sizeof(lnr_buf));
                        }
                        string_t *line;
                        vector_at(state.render, y, &line);
                        size_t len = str_length_utf8(line);
			if ((size_t)buffers.curr->col_offset < len){
				if (len >= (size_t)buffers.curr->col_offset)
					len -= buffers.curr->col_offset;
				if (len > screen_cols)
					len = screen_cols;
                                size_t n = str_bytes_between(line, buffers.curr->col_offset, buffers.curr->col_offset + len);
				const char *line_buf = str_get_buffer_at(line, buffers.curr->col_offset);
				str_concat_cstr(buf, line_buf, n);
			}
		}

		str_concat_cstr(buf, "\x1b[K\r\n", 5);
	}
}

void editor_draw_status_bar(string_t *buf){
	str_concat_cstr(buf, "\x1b[7m", 4);

	char status[256], rstatus[256], sep[256];
	int len = snprintf(status, ARRAY_SIZE(status),
			   " %s - %d lines %s",
			   buffers.curr->filename ? buffers.curr->filename : "[Unnamed]",
			   buffers.curr->num_lines,
		       	   buffers.curr->dirty ? "(modified)" : "");
	int rlen = snprintf(rstatus, ARRAY_SIZE(rstatus),
			    "Buffer:%d/%d | Row:%d | Col:%d ",
			    buffers.curr_index + 1, buffers.amount,
			    buffers.curr->cy + 1, buffers.curr->cx + 1);
	if (len > state.screen_cols)
		len = state.screen_cols;
	if (rlen > state.screen_cols - len)
		rlen = 0;

	int sep_len = state.screen_cols - len - rlen;
	snprintf(sep, ARRAY_SIZE(sep), "%*s", sep_len, " ");

	str_concat_cstr(buf, status, len);
	str_concat_cstr(buf, sep, sep_len);
	str_concat_cstr(buf, rstatus, rlen);;

	str_concat_cstr(buf, "\x1b[m", 3);
	str_concat_cstr(buf, "\r\n", 2);
}

void editor_draw_message_bar(string_t *buf){
	size_t len = ARRAY_SIZE(state.status_msg);
	if (len > (size_t)state.screen_cols)
		len = (size_t)state.screen_cols;
	str_concat_cstr(buf, "\x1b[K", 3);
	if(time(NULL) - state.status_msg_time < 5)
		str_concat_cstr(buf, state.status_msg, len);

}

void editor_set_status_message(const char *fmt, ...){
	va_list ap;
	va_start(ap, fmt);
	vsnprintf(state.status_msg, ARRAY_SIZE(state.status_msg), fmt, ap);
	va_end(ap);
	state.status_msg_time = time(NULL);
}

void editor_help(void){
	buffer_insert();
        buffers.curr->conf.line_number = false;
        static char *lines[] = {
		"Keybindings",
		"===========",
		"Ctrl + E           \t Execute command",
		"Ctrl + F           \t Format line",
		"Ctrl + K           \t Cut line",
		"Ctrl + N           \t Add buffer",
		"Ctrl + O           \t Open file",
		"Ctrl + Q           \t Kill buffer",
		"Ctrl + S           \t Save buffer",
		"Ctrl + Left Arrow  \t Move to the buffer on the left",
		"Ctrl + Right Arrow \t Move to the buffer on the right",
		"F5                 \t Reload file",
		"Alt + H            \t Display help buffer",
		"Alt + C            \t Toggle comment in the current line",
		"Alt + S            \t Search in the buffer",
		"Alt + K            \t Cut the line from the cursor to the end",
		"Alt + Up Arrow     \t Moves the current line up",
		"Alt + Down Arrow   \t Moves the current line down",
		"Alt + [1-9]        \t Go to buffer number [1-9]",
		"Alt + Left Arrow   \t Jumps to the previous word ",
		"Alt + Righ Arrow   \t Jumps to the next word ",
		"Alt + Backspace    \t Deletes a word backwards ",
		"Alt + Supr Key     \t Deletes a word forward ",
		"",
		"Commands",
		"========",
		"!quit	Exit the editor","",
		"pwd	prints the current working directory","",
		"wq	Write the buffer and close it","",
		"fwq	Same as wq, but it formats the buffer first","",
		"strip [line|buffer] 	Strips trailing whitespaces","",
		"search <string> 	jumps to the next occurence of the given string of text",
		"  search-backwards	same as search, but backwards.","",
		"replace <text> <replacement>  Replaces all instances of <text> with <replacement>","",
		"goto [line|buffer] <number>   Jumps to a certain line/buffer","",
		"format [line|buffer]	The same as Ctrl + F.","",
                "set <key> <value> Set a state.guration parameter for all new buffers","",
                "setlocal <key> <value> Set a state.guration parameter for the current buffer","",
		"help	Display the help buffer",
		NULL
	};
	for (char **line = lines; *line != NULL; line++)
		line_append(*line, -1);
}
